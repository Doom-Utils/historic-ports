...
MIDI *load_midi_data(char *dat)
{
   ...
   dat += sizeof(long);                           /* skip header chunk length */

   header_len = LONG(*((long *)dat));
   header_len = SWAPL(header_len);
   dat += sizeof(long);
