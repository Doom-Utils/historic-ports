...
void MN_Drawer(void)
{
	...
	else
	{
		...
		selName = MenuTime&16 ? "M_SLCTR1" : "M_SLCTR2";
                V_DrawPatchInDirect(x+SELECTOR_XOFFSET, y, 0,
...
static void DrawMainMenu(void)
{
	...
	frame = (MenuTime/3)%18;
        V_DrawPatchInDirect(88, 0, 0, W_CacheLumpName("M_HTIC", PU_CACHE));
        V_DrawPatchInDirect(40, 10, 0, W_CacheLumpNum(SkullBaseLump+(17-frame),
		PU_CACHE));
        V_DrawPatchInDirect(232, 10, 0, W_CacheLumpNum(SkullBaseLump+frame, PU_CACHE));
}
...
static void DrawFileSlots(Menu_t *menu)
{
	...
	for(i = 0; i < 6; i++)
	{
                V_DrawPatchInDirect(x, y, 0, W_CacheLumpName("M_FSLOT", PU_CACHE));
		if(SlotStatus[i])
		{
...
//                                 }
				...
				return(true);
//#ifndef __NeXT__
                        case KEYH_F1: // help screen
				SCInfo(0); // start up info screens
				MenuActive = true;
				return(true);
                        case KEYH_F2: // save game
				...
				slottextloaded = false; //reload the slot text, when needed
				return true;
                        case KEYH_F5: // F5 isn't used in Heretic. (detail level)
				return true;
                        case KEYH_F6: // quicksave
				if(gamestate == GS_LEVEL && !demoplayback)
				{
				...
				return true;
                        case KEYH_F8: // toggle messages
				SCMessages(0);
				return true;
                        case KEYH_F9: // quickload
				...
				return true;
                        case KEYH_F11: // F11 - gamma mode correction
				usegamma++;
				if(usegamma > 4)
				{
					usegamma = 0;
...
//#endif
	...
	if(MenuActive == false)
	{
                if(key == KEYH_ESCAPE || gamestate == GS_DEMOSCREEN || demoplayback)
		{
			MN_ActivateMenu();
			return(true);
		}
...
//                PU_CACHE), SCREENWIDTH*SCREENHEIGHT);
      V_DrawPatchInDirect(0, 0, 0, W_CacheLumpNum(W_GetNumForName("TITLE")+InfoType,
              PU_CACHE));
}
...
static void DrawSlider(Menu_t *menu, int item, int width, int slot)
{
	...
	y = menu->y+2+(item*ITEM_HEIGHT);
        V_DrawPatchInDirect(x-32, y, 0, W_CacheLumpName("M_SLDLT", PU_CACHE));
	for(x2 = x, count = width; count--; x2 += 8)
	{
                V_DrawPatchInDirect(x2, y, 0, W_CacheLumpName(count&1 ? "M_SLDMD1"
			: "M_SLDMD2", PU_CACHE));
	}
        V_DrawPatchInDirect(x2, y, 0, W_CacheLumpName("M_SLDRT", PU_CACHE));
        V_DrawPatchInDirect(x+4+slot*8, y+7, 0, W_CacheLumpName("M_SLDKB", PU_CACHE));
}
