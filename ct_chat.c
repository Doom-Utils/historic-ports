...
boolean CT_Responder(event_t *ev)
{
	...
	if(ev->data1 == KEYH_LALT || ev->data2 == KEYH_RALT)
	{
		altdown = (ev->type == ev_keydown);
		return false;
	}
	if(ev->data1 == KEYH_RSHIFT)
	{
	...
	else
	{
		if(altdown)
		{
			if(ev->data1 >= '0' && ev->data1 <= '9')
			{
				...
				CT_queueChatChar(KEYH_ENTER); //send old message
				CT_queueChatChar(chat_dest[consoleplayer]); // chose the dest.
				while(*macro)
				{
					CT_queueChatChar(toupper(*macro++));
				}
				CT_queueChatChar(KEYH_ENTER); //send it off...
				CT_Stop();
				return true;
			}
		}
		if(ev->data1 == KEYH_ENTER)
		{
			CT_queueChatChar(KEYH_ENTER);
			CT_Stop();
			return true;
		}
		else if(ev->data1 == KEYH_ESCAPE)
		{
		...
		else
		{
			...
			|| ev->data1 == KEYH_BACKSPACE || ev->data1 == '-' || ev->data1 == '=')
			{
