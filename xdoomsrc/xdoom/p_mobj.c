...
// here we maintain a linked list for things too, similar as this was
// done for lines and sectors in Boom, so that things can be found by
// their TID faster.
//
mobj_t			*tid_list[256];
...
//
void P_AddMobjToList(mobj_t *mobj)
{
    if (mobj->tid == 0)
	return;

    mobj->tnext = tid_list[mobj->tid];
    mobj->tprev = (mobj_t *)0;
    tid_list[mobj->tid] = mobj;
}

//
// Remove mobj from linked list
//
static void P_RemoveMobjFromList(mobj_t *mobj)
{
    if (mobj->tid == 0)
	return;

    if (mobj->tprev == (mobj_t *)0)
    {
	if (tid_list[mobj->tid] == mobj)
	    tid_list[mobj->tid] = mobj->tnext;
	if (mobj->tnext)
	{
	    mobj->tnext->tprev = (mobj_t *)0;
	    mobj->tnext = (mobj_t *)0;
	}
    }
    else
    {
	mobj->tprev->tnext = mobj->tnext;
	if (mobj->tnext)
	{
	    mobj->tnext->tprev = mobj->tprev;
	    mobj->tnext = (mobj_t *)0;
	}
	mobj->tprev = (mobj_t *)0;
    }
...
#if defined(BUGFIXES)
		    // fix rocket eating walls, demos will get out of sync
		    if (democompat ||
...
// P_RemoveMobj
//
mapthing_ext_t	itemrespawnque[ITEMQUESIZE];
...
void P_RemoveMobj(mobj_t *mobj)
{
    ...
    // unlink from tid list
    P_RemoveMobjFromList(mobj);
...
void P_SpawnMapThing(mapthing_ext_t *mthing)
{
    ...
    // and plutonia.wad has a thing id 0 in one map at least, doh!
    if (mthing->type == 0)
    {
    ...
    // don't abort on unknown things anymore, just filter them out
    if (i == NUMMOBJTYPES)
    {
#if 0
    ...
    // set the things tid and special
    mobj->tid = mthing->tid;
