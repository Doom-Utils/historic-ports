...
typedef struct
{
    ...
    short		width;
    short		height;
    char		pad[4];		// unused, but must be present
    short		patchcount;
...
//
// First the original code from id with the Medusa bug
//
#if !defined(BUGFIXES)
...
//
// This code was taken from Boom sources, written by Lee Killough
// for performance and to fix Medusa bug.
//
			 ...
			 int cacheheight, byte *marks)
{
	...
	if (count > 0)
	{
	    ...
	    // remember which cells in column have been drawn, so that column
	    // can later be converted into a series of posts, to fix the
	    // Medusa bug
...
//
// This code is based on Boom source, written by Lee Killough
// for performance and to fix Medusa bug.
//
void R_GenerateComposite(int texnum)
{
	...
	for (; x1 < x2; x1++)
	    if (collump[x1] == -1)	// Column has multiple patches?
	     R_DrawColumnInCache((column_t *)((byte *)realpatch +
				 LONG(cofs[x1])),
				 block + colofs[x1], patch->originy,
				 texture->height, marks + x1 * texture->height);
    }

    // next, convert multipatched columns into true columns, to fix
    // Medusa bug while still allowing for transparent regions

    source = (byte *)alloca(texture->height);	// temporary column
	...
	if (collump[i] == -1)		// process only multipatched columns
	{
	    ...
	    for (;;)
	    {
		while (j < texture->height && !mark[j])	// skip transparent cell
		    j++;
		if (j >= texture->height)		// if end of column
		{
		...
		// copy opaque cells from temporary back into the column
    ...
    // Now that the texture has been build in column cace,
    // it is purgable from zone memory.
    Z_ChangeTag(block, PU_CACHE);
}
#endif /* BUGFIXES */
...
void R_GenerateLookup(int texnum)
{
    ...
    count = (struct count *)alloca(sizeof(*count) * texture->width);
	...
	while (--i >= 0)
	{
	    ...
	    for (; x < x2; x++)
	    {
		// keep a count of the number of posts in column, to fix
		// Medusa bug while allowing for transparent multipatches
		const column_t	*col = (column_t *)((byte *)realpatch +
					LONG(cofs[x]));
	...
	while (--x >= 0)
	{
	    if (!count[x].patches)
	    {
		printf("R_GenerateLookup: column %d without a patch in texture %.8s\n",
			x, texture->name);
	    ...
	    {
		// Fix Medusa bug, by adding room for column header and
		// trailer bytes for each post in merged columns.
		// For now, just allocate conservatively 4 bytes
		// per post per patch per column, since we don't
		// know yet how many posts the merged column will
		// require, and it's bounded above by this limit.
		collump[x] = -1;	// mark lump as multipatched
		colofs[x] = csize + 3;	// three header bytes in a column
		csize += 4 * count[x].posts + 1;
...
void R_InitTextures(void)
{
    ...
    textures = Z_Malloc(numtextures * sizeof(*textures), PU_STATIC, (void *)0);
    texturecolumnlump =
      Z_Malloc(numtextures * sizeof(*texturecolumnlump), PU_STATIC, (void *)0);
    texturecolumnofs =
      Z_Malloc(numtextures * sizeof(*texturecolumnofs), PU_STATIC, (void *)0);
    texturecomposite =
      Z_Malloc(numtextures * sizeof(*texturecomposite), PU_STATIC, (void *)0);
    texturecompositesize =
      Z_Malloc(numtextures * sizeof(*texturecompositesize), PU_STATIC,
	       (void *)0);
    texturewidthmask =
      Z_Malloc(numtextures * sizeof(*texturewidthmask), PU_STATIC, (void *)0);
	...
	texturecolumnlump[i] =
	  Z_Malloc(texture->width * sizeof(*texturecolumnlump), PU_STATIC,
		   (void *)0);
	texturecolumnofs[i] =
	  Z_Malloc(texture->width * sizeof(*texturecolumnofs), PU_STATIC,
		   (void *)0);
