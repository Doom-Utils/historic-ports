...
boolean		showkey;	// checkparm of -showkeysym
...
boolean		cdaudio;	// checkparm of -cdaudio

boolean		notranslu;	// checkparm of -notrans
boolean		noendtxt;	// checkparm of -noendtxt
boolean		stripextbits;	// checkparm of -stripextbits
...
extern actionf_t smoketrail;	// original state of smoke trail
...
void D_Display(void)
{
    ...
    if (usergame)
	M_CommDevice();	// communication device
...
// *** PID BEGIN ***
// Custom process management level PWAD.  Replaces E1M1 in Doom 1.
    psdoom1wad = malloc(strlen(doomwaddir) + 1 + 11 + 1);
    sprintf(psdoom1wad, "%s/psdoom1.wad", doomwaddir);

// Custom process management level PWAD.  Replaces MAP01 in Doom 2.
    psdoom2wad = malloc(strlen(doomwaddir) + 1 + 11 + 1);
    sprintf(psdoom2wad, "%s/psdoom2.wad", doomwaddir);
// *** PID END ***
    ...
    // yeah, well, we do abort, PWADs alone won't work.
