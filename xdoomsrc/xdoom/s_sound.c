...
// flags if no sound or music is wanted
...
//
void S_Init(int sfxVolume, int musicVolume, int cdVolume)
{
  int		i;

  printf("S_Init: default sfx volume %d\n", sfxVolume);
  printf("S_Init: default music volume %d\n", musicVolume);
  printf("S_Init: default CD volume %d\n", cdVolume);
  printf("S_Init: using %d sound channels\n", numChannels);

  I_SetChannels();
  S_SetSfxVolume(sfxVolume);
  S_SetMusicVolume(musicVolume);
  S_SetCdVolume(cdVolume);
...
//
// This is from Hexen, needed for XDoomPlus
//
...
void S_SetCdVolume(int volume)
{
    if (volume < 0 || volume > 127)
    {
	I_Error("Attempt to set CD volume at %d",
		volume);
    }

    I_SetCdVolume(volume);
    snd_CdVolume = volume;
}
