...
//	 mostly used with 4Frond OSS sound driver for UNIX.
//
//-----------------------------------------------------------------------------
...
/* needed for definition of bzero */
#ifdef __FreeBSD__
#include <string.h>
#endif
...
void grabdata(int c, char **v)
{
    ...
    {
	fprintf(stderr, "sndserver: Could not find wadfile anywhere\n");
	exit(-1);
    }
    ...
    // another GROSS HACK, yuck
    if (!got_chgun)
    {
	S_sfx[sfx_chgun].data = S_sfx[sfx_pistol].data;
	lengths[sfx_chgun] = lengths[sfx_pistol];
	fprintf(stderr, "sndserver: linking chaingun sound to pistol sound\n");
    }
    else
	fprintf(stderr, "sndserver: found chaingun sound\n");
}
		...
		if (rc > 0)
		{
			...
			switch (commandbuf[0])
			{
			  ...
			  case 's':
			  {
			      ...
			      read(0, commandbuf, 3);
			      commandbuf[2] = 0;
			      fd = open((char*)commandbuf, O_CREAT|O_WRONLY,
					0644);
			      commandbuf[0] -= commandbuf[0]>='a' ?
					'a'-10 : '0';
			      commandbuf[1] -= commandbuf[1]>='a' ?
					'a'-10 : '0';
			      sndnum = (commandbuf[0]<<4) + commandbuf[1];
